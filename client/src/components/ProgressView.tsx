...
export function ProgressView(props: Props) {
  ...
  type ScoreKey = 'avg_score' | 'avg_accuracy' | 'avg_fluency' | 'avg_integrity';
  ...
  function xs(i: number): string {
    return (pad.left + (sessions.length > 1 ? (i / (sessions.length - 1)) * pw : pw / 2)).toFixed(
      1,
    );
  }
  ...
  /** 画折线 */
  function polylineD(key: ScoreKey): string {
  ...
  return (
    <div className="progress-panel">
      <div className="report-header-bar">
        <button onClick={props.onClose} className="ctrl-btn">
          ← 返回对话
        </button>
      </div>
          ...
          <div className="report-stat-grid">
            ...
            <div className="report-stat-card">
              ...
              <div className="report-stat-value">
                {sessions.reduce((s, x) => s + x.turn_count, 0)}
              </div>
            </div>
            <div className="report-stat-card">
              ...
              <div className="report-stat-value accent">
                {sessions.length > 0
                  ? Math.round(
                      sessions.slice(-3).reduce((s, x) => s + x.avg_score, 0) /
            ...
            <div className="report-stat-card">
              ...
              <div className="report-stat-value accent">
                {sessions.length > 0
                  ? Math.round(Math.max(...sessions.map((s) => s.avg_score)))
                  : '-'}
                分
              </div>
            </div>
              ...
              <section className="report-section">
                ...
                <div className="radar-legend" style={{ marginTop: 12, justifyContent: 'center' }}>
                  <div className="radar-legend-item">
                    <span className="radar-legend-dot" style={{ background: '#3b82f6' }} />
                    <span className="radar-legend-label">准确度</span>
                  </div>
                  <div className="radar-legend-item">
                    <span className="radar-legend-dot" style={{ background: '#22c55e' }} />
                    <span className="radar-legend-label">流利度</span>
                  </div>
                  <div className="radar-legend-item">
                    <span className="radar-legend-dot" style={{ background: '#f59e0b' }} />
                    <span className="radar-legend-label">完整度</span>
                  </div>
                </div>
              </section>
              ...
              <section className="report-section">
                ...
                <svg viewBox={`0 0 ${w} 160`} className="progress-svg">
                  {sessions.map((s, i) => {
                    ...
                    const barH = Math.max(
                      2,
                      (s.turn_count / Math.max(1, Math.max(...sessions.map((x) => x.turn_count)))) *
                        100,
                    );
                    ...
                    return (
                      <g key={`bar-${i}`}>
                        <rect
                          ...
                          rx="2"
                          fill="var(--accent)"
                          fillOpacity={s.turn_count > 0 ? 0.7 : 0.2}
                        />
                        <text
                          x={Number(xs(i))}
                          y={134}
